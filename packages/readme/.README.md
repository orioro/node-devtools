...
Overrides the displayed signature and parameter/return types. Use this when the
exported symbol's TypeScript type doesn't match its actual call signature — for
example, when your library wraps functions in metadata tuples or factory
patterns, but callers interact with the inner function type.

```ts
// Internal pattern: functions are bundled with a cache key
type Registered<T> = readonly [fn: T, cacheKey: string]

type FnSearch = (query: string, limit?: number) => Promise<string[]>

/**
 * Searches entries matching the query.
 * @signature function search(query: string, limit?: number): Promise<string[]>
 * @param query - search string
 * @param limit - max number of results
 * @returns matching entries
 * @public
 */
export const search: Registered<FnSearch> = [
  async (query, limit = 10) => fetchResults(query, limit),
  'search',
]
```

Without `@signature`, the docs would show `const search: Registered<FnSearch>`.
With it, they show `function search(query: string, limit?: number): Promise<string[]>`.

The signature string is parsed to extract param names and types. `@param`
descriptions are merged by matching param names. Types referenced in the
signature (e.g. `FnSearch`) are resolved against the actual TypeScript program
and included in the type definitions section, just as they would be for inferred
types.

If the signature string cannot be parsed, the entry falls back to normal type
inference.
